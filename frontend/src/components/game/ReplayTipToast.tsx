      ...
      <div className="rounded-xl border border-amber-500/40 bg-cc-surface/95 shadow-2xl backdrop-blur-sm p-4">
        <div className="flex items-start gap-3">
            ...
            <Lightbulb
              className={
                insight.impact === 'high'
                  ...
                  : 'w-5 h-5 text-cc-gold'
              }
            />
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-[10px] uppercase tracking-wider text-cc-muted">Turn {insight.turn}</span>
              {insight.impact === 'high' && (
            ...
            <p className="text-sm text-cc-text font-medium mb-1.5">{insight.title}</p>
            <p className="text-xs text-cc-muted leading-relaxed mb-2">{insight.explanation}</p>
          ...
          <button
            ...
            className="shrink-0 p-1 rounded text-cc-muted hover:text-cc-text hover:bg-white/5 transition-colors"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
