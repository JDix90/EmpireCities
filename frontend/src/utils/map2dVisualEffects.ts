...
function runEraAdvanceEffect(
  ...
  const isBoardTransform = event.kind === 'board_transform';
  const eraName = eraAdvanceDisplayName(event.variant);
  const title = new PIXI.Text(isBoardTransform ? 'THE WORLD TURNS' : isFrontier ? 'NEW FRONTIER' : `${eraName.toUpperCase()} ERA`, {
    fontSize: 28,
    fill: '#fff8dc',
    fontWeight: 'bold',
    stroke: '#5c4000',
  ...
  const subtitle = new PIXI.Text(isBoardTransform ? `${eraName} · A New World` : isFrontier ? 'Ripe for the Taking' : 'Civilization Ascends', {
    fontSize: 13,
    fill: '#ffd700',
    fontWeight: 'bold',
    letterSpacing: 2,
...
export function playMap2dVisualEffect(
  ...
  if (event.kind === 'era_advance' || event.kind === 'frontier_unlock' || event.kind === 'board_transform') {
