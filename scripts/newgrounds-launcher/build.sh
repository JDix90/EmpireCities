#!/usr/bin/env bash
# Package the Newgrounds launcher shell for upload. See README.md.
#
...
OUT_DIR="${HERE}/dist"
OUT="${OUT_DIR}/borderfall-newgrounds.zip"

mkdir -p "${OUT_DIR}"
rm -f "${OUT}"

# index.html must sit at the ARCHIVE ROOT. Newgrounds looks for it there and
# will not find it one directory down — a zip of the FOLDER fails, a zip of the
# folder's CONTENTS works.
