...
| `storeV2Enabled` | `STORE_V2_ENABLED` | off | Store overhaul: a banner slot separate from the frame (`equipped_banner`), unequipping, and every equipped frame and banner drawn on the profile, with previews in My Loadout. Off = the store, profiles and matches exactly as before |
