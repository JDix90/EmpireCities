...
| `storeV2Enabled` | `STORE_V2_ENABLED` | off | Store overhaul: a banner slot separate from the frame (`equipped_banner`), unequipping, and every equipped cosmetic drawn where players see each other: frames and banners on the profile and beside names in matches (players list, combat cards, final standings), dice skins in combat, map markers on capitals (2D map and globe), and the new store page (previews, rarity and price on every card, the gold still needed, equip after buying, the loadout with Default). The era sets (migration 046, `cosmetics.cosmetic_set`) are listed and sold only with it on. A match takes its players' cosmetics when it starts; turning the flag off hides them at once, running games included. Off = the store, its catalog, profiles and matches exactly as before |
