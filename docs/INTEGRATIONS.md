...
- **Permission is asked only inside a click** — `enableWebPush()` from Settings → Notifications → This browser, the lobby's opt-in card, or the prompt right after an async turn ends (`offerPushAfterAsyncTurn`, once per page load; "Not now" on either card snoozes both for 30 days). Guests included: a browser token needs no email, and the token routes accept guest sessions. The prompt is one-shot per origin and browsers remember a refusal; a gesture-less prompt on login once burned it on every deployment without Firebase. Init only completes a registration that was already granted. On an iPhone browser tab (web push exists only in a Home Screen app) the controls give Add-to-Home-Screen advice instead.
