...
**A victory no normal game had (fixed later).** That gate exposed the bigger gap behind it: `applyOrbitGatedVictoryDefaults` added `lunar_hegemony` to a *blank* victory list only, and the lobby and Quick Match always send a list. So the Hegemony, and with it the contest rule, never ran in an ordinary game at all. It now joins whatever list the caller sent whenever its phase is baked and the board has a Moon to hold — it is part of the Moon Race, not a lobby choice, and like the rest of the package its only switch is the operator's phase flag. The threshold-60 backstop and Lane Sovereignty still fill a blank list only. The client mirrors the contest rule too (`moonContestOpen` in `frontend/src/utils/orbitAccess.ts`), so the Space Program tracker and the Moon-access hints show the shorter requirement instead of a ladder the server no longer asks for. Games created before the change keep the settings they were created with.
...
The Hegemony needed splitting out from the orbit-gated defaults: it must never bring the threshold-60 / 90-turn backstop with it, which exists for a game orbit-gated from turn one and would turn a marathon climb into a different game. In practice an ascending game does not get `lunar_hegemony` at all: the create route adds it only on a board with lunar tiles, and with the board transform parked a climb stays on its moonless starting board, where the route could never fire.
