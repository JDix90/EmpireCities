...
**packages/warfront-sim** — integer-only deterministic sim core for the experimental Warfront RTS mode (16.16 fixed point, seeded RNG, cell-grid terrain + flow-field pathing, golden replay tests); no runtime deps, lint-enforced determinism rules in the root `eslint.config.mjs`. The terrain asset it walks on is generated by `pnpm run build:warfront-terrain` into `database/warfront/` (see the package README for why that location). Not used by either app yet.
