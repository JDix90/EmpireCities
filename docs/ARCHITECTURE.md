...
- **Postgres** (45 migrations in [database/migrations/](../database/migrations)): `users` (identity + progression columns: xp/level/gold/streaks/`is_guest`), `games`/`game_players` (lobby + results), `game_states` (state backups), `user_ratings` (Glicko-2 `mu/phi/sigma` per `solo|ranked`), `refresh_tokens` (rotation), `maps`/`map_ratings` (**map documents as JSONB** — migrated from MongoDB), achievements/cosmetics/quests/seasons, `admin_config` (feature-flag overrides).
