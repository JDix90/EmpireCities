...
- **Objectives**: `puzzleObjective.ts` is the registry (`condition`, hold-through-AI-turn, lapse-is-failure, timeout outcome); the socket consults it every turn. Capture-shaped verbs require holding the target through the AI's reply, unless the capture left no rival to reply: `finalizeGame` settles a day the human won outright on the final board (`isObjectiveMetAtConquest`). Winning the game is not winning the day. A conquest that ends the game before the objective is met is a lost run, paid out as a loss (`creditedWinnerIds`: XP, rank, rating, win streak, gold, achievements), and `game:over` carries `daily_result` so the result screen says so.
...
- **Postgres** (44 migrations in [database/migrations/](../database/migrations)): `users` (identity + progression columns: xp/level/gold/streaks/`is_guest`), `games`/`game_players` (lobby + results), `game_states` (state backups), `user_ratings` (Glicko-2 `mu/phi/sigma` per `solo|ranked`), `refresh_tokens` (rotation), `maps`/`map_ratings` (**map documents as JSONB** — migrated from MongoDB), achievements/cosmetics/quests/seasons, `admin_config` (feature-flag overrides).
...
**packages/warfront-sim** — integer-only deterministic sim core for the experimental Warfront RTS mode (16.16 fixed point, seeded RNG, cell-grid terrain + flow-field pathing, golden replay tests); no runtime deps, lint-enforced determinism rules in the root `eslint.config.mjs`. The terrain asset it walks on is generated by `pnpm run build:warfront-terrain` into `database/warfront/` (see the package README for why that location). The frontend depends on it for the admin-only solo match at `/admin/warfront`, which runs the sim in the browser against its policy bots; the backend deliberately does not, and serves only the admin-guarded status and terrain endpoints (`modules/admin/warfrontStatus.ts`). No match host or netcode exists.

## CI & testing

[.github/workflows/ci.yml](../.github/workflows/ci.yml) runs on every PR and push to `main`:

- **secrets job** — `scripts/check-secrets.sh` scans every tracked file for live-looking credentials. Dependency-free, so it reports in seconds rather than waiting on the install/build jobs.
- **docs job** — `scripts/check-docs.sh` re-verifies the load-bearing claims in the maintained docs against the code: documented env vars, the migration count, relative links, and a sweep for retired state-model phrasing. Same shape as `secrets`. See [docs index → Keeping docs accurate](README.md#keeping-docs-accurate).
- **backend job** — Redis 7 and Postgres 16 service containers; shared build → `tsc` → migrate + seed maps into the Postgres service → full vitest suite with `REDIS_TEST=1` and `PG_TEST=1`, which switch on every test file gated on them: `REDIS_TEST` for the files that talk to a real Redis, `PG_TEST` for those that need the migrated Postgres, and both for a file that needs both (such as `spectatorRedaction.test.ts`) → `validate:maps` → ESLint. Nothing in the suite is skipped in CI.
